...
const Cart = () => {
  ...
  const totalPrice = cartItems.reduce((total, item) => total + Number(item.price) * Number(item.Qty || 0), 0);
  ...
  return (
    <>
      ...
      <div className={`fixed right-0 w-full  overflow-y-auto lg:w-[25%] top-0 h-[100%] bg-white z-20 ${show ? "translate-x-0" : "translate-x-full"} transition-all duration-500`}>
        ...
        <div className='px-5'> {cartItems.length > 0 ? (
          cartItems.map((food) => (
            <ItemCard
              key={food.id}
              name={food.name}
      ...
      <FaShoppingCart 
        ...
        className={`fixed bottom-4 right-4 text-white text-6xl bg rounded-full shadow-2xl px-3 py-1 cursor-pointer ${totalqty > 0 ? "animate-bounce delay-500 transition-all" : ""}`}
      />
    </>
  );
}
